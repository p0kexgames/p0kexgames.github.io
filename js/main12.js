...
$(".aviso").html(" ");
  ...
  //// FORMATA O CAMPO DOS LOOTS PARA EXIBIR CORRETAMENTE
     ...
     result = [];  
     for( i=0; i<len; i++ ) {          
      result[i] = '<li class="loot-li"><i class="item-img">.</i> '+text[i]+'//</li>';
      if(result.indexOf(text[i]) == -1) result.push(text[i]); 
     }
 ...
 var map = {};
  ...
  $('ul.loots li.loot-li span.itemPerc').each(function( ind ){   
       var text = $(this).html().split('--'),
           ...
           result = [];  
       for( i=0; i<len; i++ ) {          
           result[i] = '<i class="item-img">.</i><span class="itemNome">'+text[0]+'</span> <span class="itemPercentual">'+text[1]+'</span>';
