...
function updateDivTimer(td, tz, sr) {
	...
	document.getElementById(timerDivId0[temp]).innerHTML = "<div>&nbsp;</div><strong>" + tzStrings[temp] + "</strong><br>Evento inicia em: " + eventTime.local().format("YYYY-MM-DD HH:mm:ss") + " " + moment.tz(moment.tz.guess()).zoneAbbr();
	Cookies.set('userTimeZones', allTimeZones.join('|'), {expires: 10});
	Cookies.set('userCities', tzStrings.join('|'), {expires: 10});
}
countdownTimer();
		...
		for (var cnt = 0; cnt < allTimeZones.length; cnt++) {
			if(allTimeZones[cnt] != "none") {
				...
				if (distance >= 0) {
					...
					document.getElementById(timerDivId1[cnt]).innerHTML = "Evento começara em: " + ("0" + days).slice(-2) + "d " + ("0" + hours).slice(-2) + "h " + ("0" + minutes).slice(-2) + "m " + ("0" + seconds).slice(-2) + "s ";		
				}
