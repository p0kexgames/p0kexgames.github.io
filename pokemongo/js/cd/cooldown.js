function distance() {
	try {
		...
		if ((lat1 == lat2) && (lon1 == lon2)) {
			dist = 0;
			result = "A distância entre " + coord1 + " (ponto A) e " + coord2 + " (ponto B)  é " + dist + " km";
		}
		else {
			...
			dist = dist * 60 * 1.1515;
			dist = dist * 1.609344;
			result = "A distância entre " + coord1 + "(ponto A) e " + coord2 + " (ponto B)  é " + Math.round(dist*100) /100 + " km";
		...
		result += " e o tempo de espera (Cooldown) é " + ("0" + hours).slice(-2) + "h " + ("0" + minutes).slice(-2) + "m";
		...
		document.getElementById("cooldownResult").innerHTML = "<div>&nbsp;</div>Por favor, verifique suas coordenadas novamente." + err;
	}
}
